...
class TcpServer {
 public:
  TcpServer(std::string local_name, std::string remote_name)
      : io_service_(),
        socket_(io_service_),
  ...
  TcpServer(const std::string& host, short port, short timeout, std::string local_name, std::string remote_name)
      : io_service_(),
        socket_(io_service_),
...
int main() {
  try {
    TcpServer server("localhost", 12345, 1000, "local", "remote");  // 连接到本地服务端12345端口
    server.StartRead();
