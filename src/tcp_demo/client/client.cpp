...
class TcpClient {
 public:
  TcpClient(std::string local_name, std::string remote_name)
      : io_service_(),
        socket_(io_service_),
  ...
  TcpClient(const std::string& host, short port, short timeout, std::string local_name, std::string remote_name)
      : io_service_(),
        socket_(io_service_),
        deadline_(io_service_),
        timeout_(timeout),
  ...
  void StartConnect() {
    ...
    deadline_.expires_after(boost::asio::chrono::seconds(timeout_));
    ...
    async_connect(socket_, endpoint_iterator_,
                  boost::bind(&TcpClient::HandleConnect, this, conn_result, boost::placeholders::_1));
  ...
  void StartConnect(const std::string& host, short port, short timeout) {
    if (this->IsOpen()) {
      this->Close();
    }
  ...
  void StartWrite(const std::string& msg, short timeout) {
    std::cout << "clint start write..." << std::endl;

    boost::optional<boost::system::error_code> timer_result;
    deadline_.expires_after(boost::asio::chrono::seconds(timeout));
    deadline_.async_wait([&timer_result](const boost::system::error_code& error) { timer_result.reset(error); });

    boost::optional<boost::system::error_code> write_result;
    async_write(socket_, boost::asio::buffer(msg.c_str(), msg.size()),
                boost::bind(&TcpClient::HandleWrite, this, write_result, boost::placeholders::_1));
  ...
  void StartRead(std::string& msg, size_t size, short timeout) {
    ...
    socket_.async_read_some(
        ...
        boost::bind(&TcpClient::HandleRead, this, read_result, boost::placeholders::_1, boost::placeholders::_2));

    io_service_.restart();
    while (io_service_.run_one()) {
      if (read_result.has_value()) {
        deadline_.cancel();
        msg = std::string(data_, size);
      } else if (timer_result.has_value()) {
        socket_.cancel();
      }
    }
  ...
  void HandleRead(boost::optional<boost::system::error_code>& read_result, const boost::system::error_code& error,
                  size_t length) {
...
int main() {
  try {
    TcpClient client("localhost", 12345, 200, "local", "remote");  // 连接到本地服务端12345端口
    client.StartWrite("hello world", 200);

    std::string msg{};
    client.StartRead(msg, 16, 200);
